...
( function( jQuery ) {
    ...
    window.awbOffCanvas = {
        ...
        open_off_canvas: function( id, setCount = true, toggle = false ) {
            ...
            if ( 'off' === options.status_css_animations || ( 'desktop' === options.status_css_animations && window.cssua.ua.mobile ) ) {
                animation = false;
            }
            if ( animation ) {
                ...
                if ( 'sliding-bar' === options.type && 'push' === options.transition && 'top' === options.position ) {
                    const pushCss = { 'transition': 'margin-top ' + options.enter_animation_speed + 's' };
                    pushCss.marginTop = offCanvas.outerHeight() + 'px';
                    ...
                    // Sticky Header.
					const stickyHeader = jQuery( '.fusion-tb-header' ).find( '.fusion-sticky-container.fusion-container-stuck' );
					if ( stickyHeader.length ) {
						stickyHeader.each( function() {
							jQuery( this ).attr( 'data-top', jQuery( this ).css( 'top' ) );
							jQuery( this ).css( { 
								'top': 'calc(' + pushCss.marginTop + ' + ' + jQuery( this ).attr( 'data-top' ) + ')',
								'transition': 'top ' + options.enter_animation_speed + 's'
							} );
						} );
					}
                }
        ...
        close_off_canvas: function( id, setCount = true ) {
            ...
            if ( animation ) {
                ...
                if ( 'sliding-bar' === options.type && 'push' === options.transition && 'top' === options.position ) {
                    const pushCss = { 'transition': 'margin-top ' + options.enter_animation_speed + 's', marginTop: 0 };
                    let pushedEl = '#wrapper';
                    ...
                    // Sticky header.
                    const stickyHeader = jQuery( '.fusion-tb-header' ).find( '.fusion-sticky-container' );
                    if ( stickyHeader.length ) {
                        stickyHeader.each( function() {
							const self = this;
                            if ( 'undefined' !== typeof jQuery( this ).attr( 'data-top' ) ) {
                                jQuery( this ).css( 'top', jQuery( this ).attr( 'data-top' ) );
                                jQuery( this ).removeAttr( 'data-top' );

								setTimeout( function() {
									jQuery( self ).css( 'transition', '' );
								}, options.enter_animation_speed * 1000 );
                            }
                        } );
                    }
                }
            ...
            this.moveOCContentToPostCard( wrap );
        },
        ...
        /**
         * Moves the post card specific Off Canvas content to the Off Canvas content area.
         *
         * @since 3.14
         * @param {int} id
         * @param {Object} $button
         * @return {void}
         */
        movePostCardOCContentToOC( id, $button ) {
            const $parentColumn = $button.closest( '.fusion-layout-column:not(.fusion_builder_column_inner)' );
	
            if ( $parentColumn.length && $parentColumn.hasClass( 'post-card' ) && $parentColumn.children( '.awb-post-card-off-canvas-content' ).length ) {
                const $wrap = jQuery( '.awb-off-canvas-wrap[data-id="' + id + '"]' );

                if ( $wrap.length ) {
                    const $contentWrapper = $wrap.find( '.off-canvas-content' ),
                        $newContent       = $parentColumn.children( '.awb-post-card-off-canvas-content' );

                    $contentWrapper.data( 'post-card', $newContent ).empty();
                    $newContent.children().appendTo( $contentWrapper );

                    // Reinit or init some dynamic stuff.

                    // Auto triggered stuff.
                    jQuery( window ).trigger( 'fusion-dynamic-content-render', $contentWrapper );

                    // Make videos fit.
                    $contentWrapper.find( '.full-video, .video-shortcode, .wooslider .slide-content' ).fitVids();

                    // Set the featured image slider to slide 1.
                    $contentWrapper.find( '.fusion-flexslider, .flexslider' ).each( function() {
                        if ( 1 < jQuery( this ).find( '.slides li' ).length ) {
                            jQuery( this ).flexslider( 0 );
                        }
                    } );

                    // Redraw open street map.
                    $contentWrapper.find( '.awb-openstreet-map' ).trigger( 'awb_openstreetmap/updateMap' );
                }
            }
        },

        /**
         * Moves the Off Canvas content back to the Post Card it originated from.
         *
         * @since 3.14
         * @param {Object} $wrap
         * @return {void}
         */
        moveOCContentToPostCard( $wrap ) {
            const $contentWrapper = $wrap.find( '.off-canvas-content' ),
                $postCardContent  = 'undefined' !== typeof $contentWrapper.data( 'post-card' ) ? $contentWrapper.data( 'post-card' ) : {};

            if ( $postCardContent.length ) {
                $contentWrapper.removeData( 'post-card' );
                $contentWrapper.children().appendTo( $postCardContent );
            }
        }
    };
    ...
    jQuery( 'body' ).on( 'click', 'a[href^="#awb-oc__"]', function( event ) {
        event.preventDefault();

        const href = jQuery( this ).attr( 'href' ) || '',
              id   = href.split( '__' )[ 1 ];

        window.awbOffCanvas.movePostCardOCContentToOC( id, jQuery( this ) );

        window.awbOffCanvas.open_off_canvas( id, false, true );
    } );

    // Open with link href.
    jQuery( 'body' ).on( 'click', 'a[href^="#awb-open-oc__"]', function( event ) {
        event.preventDefault();
        const   href = jQuery( this ).attr( 'href' ) || '',
        id = href.split( '__' )[ 1 ];
    ...
    // Close with link href.
    jQuery( 'body' ).on( 'click', 'a[href^="#awb-close-oc__"]', function( event ) {
        event.preventDefault();
